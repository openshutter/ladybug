 ...
 * 
 * triggerDuration: ms to hold "full-press" trigger closed
 * preshootDuration: ms to hold "half-press" trigger closed
 *                   (useful for slow autofocus, exposure, etc)
 * fast: skip extra led blinks and other delays for minimal latency
 * 
 */
void triggerOutput(int triggerDuration = 500, int preshootDuration = 0, bool fast = false) {
  ...
  if (!fast) { blinkLed(100, 4); }
  if (!fast) { blinkLed(400); }
